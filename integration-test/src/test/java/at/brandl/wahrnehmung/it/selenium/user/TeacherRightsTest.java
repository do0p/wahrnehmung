...
public class TeacherRightsTest {
	private static TestContext testContext;

	@Before
	public void setUp() {
		testContext = TestContext.getInstance();
		testContext.login(Constants.TEACHER);
	}
	
	@Test
	public void availableNavigation() {
		Assert.assertNotNull(testContext.getLink(Constants.NOTICE_LINK));
		Assert.assertNotNull(testContext.getLink(Constants.FORM_LINK));
		Assert.assertNotNull(testContext.getLink(Constants.SEARCH_LINK));
		Assert.assertNotNull(testContext.getLink(Constants.INTERACTION_LINK));
		Assert.assertNotNull(testContext.getLink(Constants.DOCUMENTATION_LINK));
		...
		Assert.assertNotNull(testContext.getLinkByDebugId(DialogueAdminPage.PAGE_NAME));
