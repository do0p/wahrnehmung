...
{
  private final AuthorizationServiceAsync authorizationService = (AuthorizationServiceAsync)GWT.create(AuthorizationService.class);
  ...
  public AuthorizationAdmin()
  {
    ...
    this.adminCheckBox = new CheckBox("Admin");
    this.seeAllCheckBox = new CheckBox("Betreuer");
    this.editSectionCheckBox = new CheckBox("Section-Admin");
    this.saveButton = new Button("anlegen");
    this.deleteButton = new Button("entf");
  ...
  private void rebuildUsersList()
  {
    ...
    this.authorizationService.queryAuthorizations(new AsyncCallback<Collection<Authorization>>()
    {
      public void onFailure(Throwable caught)
      {
        AuthorizationAdmin.this.dialogBox.setErrorMessage(caught.getLocalizedMessage());
        AuthorizationAdmin.this.dialogBox.setDisableWhileShown(new FocusWidget[] { AuthorizationAdmin.this.saveButton });
      ...
      public void onSuccess(Collection<Authorization> result)
      {
        for (Authorization authorization : result) {
          AuthorizationAdmin.this.users.addItem(authorization.getEmail(), 
            authorization.getUserId());
          AuthorizationAdmin.this.authorizations.put(authorization.getUserId(), 
            authorization);
        }
      }
    });
  ...
  public class CancelClickHandler
  ...
  public class DeleteClickHandler
    implements ClickHandler
  {
    ...
    public void onClick(ClickEvent event)
    {
      ...
      AuthorizationAdmin.this.authorizationService.deleteAuthorization(email, 
        new AsyncCallback<Void>()
      {
        public void onFailure(Throwable caught)
        {
          AuthorizationAdmin.this.dialogBox.setErrorMessage(caught
            .getLocalizedMessage());
          AuthorizationAdmin.this.dialogBox.setDisableWhileShown(new FocusWidget[] { AuthorizationAdmin.this.deleteButton });
          AuthorizationAdmin.this.dialogBox.center();
        }
    ...
    implements ClickHandler
  {
    ...
    public void onClick(ClickEvent event)
    {
      ...
      aut.setEmail(email);
      aut.setAdmin(AuthorizationAdmin.this.adminCheckBox.getValue().booleanValue());
      aut.setSeeAll(AuthorizationAdmin.this.seeAllCheckBox.getValue().booleanValue());
      aut.setEditSections(AuthorizationAdmin.this.editSectionCheckBox.getValue());

      AuthorizationAdmin.this.authorizationService.storeAuthorization(aut, 
        new AsyncCallback<Void>()
      {
        public void onFailure(Throwable caught)
        {
  ...
  public class UserClickHandler implements ClickHandler
  {
    ...
    public void onClick(ClickEvent event) {
      int selectedIndex = AuthorizationAdmin.this.users.getSelectedIndex();
      if (selectedIndex < 0) {
        return;
      }
      Authorization authorization = (Authorization)AuthorizationAdmin.this.authorizations.get(AuthorizationAdmin.this.users
        .getValue(selectedIndex));
      AuthorizationAdmin.this.userBox.setText(AuthorizationAdmin.this.users.getItemText(selectedIndex));
      AuthorizationAdmin.this.adminCheckBox.setValue(Boolean.valueOf(authorization.isAdmin()));
      AuthorizationAdmin.this.seeAllCheckBox.setValue(Boolean.valueOf(authorization.isSeeAll()));
      AuthorizationAdmin.this.editSectionCheckBox.setValue(Boolean.valueOf(authorization.isEditSections()));

      AuthorizationAdmin.this.saveButton.setHTML("&auml;ndern");
