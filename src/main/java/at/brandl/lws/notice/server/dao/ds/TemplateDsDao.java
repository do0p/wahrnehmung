...
public class TemplateDsDao extends AbstractDsDao {
	...
	public List<GwtTemplate> getTemplates() {
		...
		if (dirty) {
			...
			templates = (List<GwtTemplate>) getCache(TEMPLATE_MEMCACHE).get(ALL_TEMPLATES);
			if (templates == null) {
				templates = updateCacheFromDatastore();
			}
		}
